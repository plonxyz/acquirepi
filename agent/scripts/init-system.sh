...
setup_ethernet() {
    ...
    led_control 1  # Pi 5 ACT LED is active-low: 1=off, 0=on
    gpio -g write 26 0
    gpio -g mode 6 out
    gpio -g write 6 1
}
...
main() {
    ...
    # Wait up to 30 seconds for ethernet connection
    echo "Waiting for ethernet connection..."
    ethernet_timeout=30
    ethernet_wait=0
    ethernet_connected=false

    while [ $ethernet_wait -lt $ethernet_timeout ]; do
        if check_ethernet_connection; then
            echo "Ethernet connection established after ${ethernet_wait} seconds."
            ethernet_connected=true
            break
        fi
        sleep 2
        ethernet_wait=$((ethernet_wait + 2))
    done
    ...
    if [ "$ethernet_connected" = true ]; then
        ...
        echo "No ethernet connection detected. Waiting for Wi-Fi networks to be scanned..."
        sleep 10
        setup_wifi
    fi
